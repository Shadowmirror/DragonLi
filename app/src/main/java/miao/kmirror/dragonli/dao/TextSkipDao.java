...
public class TextSkipDao {
    ...
    public int updateById(int id, int appOrWeb, int appOrWebId) {
        ContentValues values = new ContentValues();
        values.put("appOrWeb", appOrWeb);
        values.put("appOrWebId", appOrWebId);
        int update = LitePal.update(TextSkip.class, values, id);
        return update;
    }
